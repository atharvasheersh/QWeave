...
def _two_qubit_count(circuit: object) -> int:
    return sum(len(instruction.qubits if hasattr(instruction, "qubits") else instruction[1]) == 2 for instruction in circuit.data)
...
def circuit_metrics(circuit: object, original: object | None = None, runtime: float | None = None) -> dict:
    ...
    return {
        ...
        "swap_count": sum((instruction.operation if hasattr(instruction, "operation") else instruction[0]).name == "swap" for instruction in circuit.data),
